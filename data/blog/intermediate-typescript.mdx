---
title: Intermediate Typescript Part 1
date: '2022-01-25'
...
Before Typescript when working our codebase I found myself having to read a lot of code, api schemas, and tests just to see what fields actually existed.
So during the transition I tried my hardest to make the types I made as good as they could be.
Converting to Typescript and making big interface/types with many optional fields does not buy you much other than typo prevention and basic autocomplete.

Typescript is a great language to get your feet wet with stronger type systems you might find in languages like ocaml/haskell/rust and many others.
You have many escape hooks when trying to type something is not worth the effort. Also you can incrementally make your types stronger as you gain more experience with the code base.
...
[Literal Types](https://www.typescriptlang.org/docs/handbook/2/everyday-types.html#literal-types) on the other hand are a much stronger restricion on what the allowed values are
...
[Union Types](https://www.typescriptlang.org/docs/handbook/2/everyday-types.html#union-types) allow you to say a type is either foo or bar or number or string...
...
When combined with literals you can make very strongly defined like so
...
your editor will provide autocomplete on the method field, and you get a compile error if you try to use an arbiarty string as the method param.

## Restricting Unions

The last part is an important topic to handle, how do you properly use a general typed string as a string literal? Typescript allows this in a few ways
...
This manual checking is fine but if you have a more complex type or a union with many possible values this gets unweilding somewhat fast.
...
when true it nows the input param `method` is a `MethodType` and can be used as such.
Type predicates are a good simple way to to encode any runtime checks into the type system.

## Discrimnated unions

Now unions of basic literals are quite powerful but they get more when you make unions of objects.
...
Now in the future if we added an event with a type of `NewEvent` it would fall through to the default case,
...
there are many other really cool typescript features, like generic, mapped types and conditonal types.
